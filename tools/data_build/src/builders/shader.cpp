...
namespace rn
{
    ...
    bool WriteShaderBytecodeToHeader(const DataBuildContext& ctxt, Span<const ShaderCompilationResult> results, Span<const EntryPoint> entryPoints, TargetAPI api, Vector<std::string>& outFiles)
    {
        ...
        return (WriteDataToDisk(ctxt, extension, { reinterpret_cast<uint8_t*>(outString.data()), outString.length()}, true, outFiles) == 0);
    }

    void WriteShaderPDBs(const DataBuildContext& ctxt, Span<const ShaderCompilationResult> results, Span<const EntryPoint> entryPoints, TargetAPI api, Vector<std::string>& outFiles)
    {
        for (const ShaderCompilationResult& result : results)
        {
            ...
            WriteDataToDisk(ctxt, extension, { static_cast<uint8_t*>(pdb->GetBufferPointer()), pdb->GetBufferSize()}, false, outFiles);
        }
    }

    void ParseIncludeDependencies(const DataBuildContext& ctxt, IDxcCompiler3* compiler, IDxcIncludeHandler* includeHandler, const std::wstring& absoluteSourcePath, const DxcBuffer& sourceBuffer, const Vector<LPCWSTR>& inArgs, Vector<std::string>& outFiles)
    {
    ...
    bool CompileShader(const DataBuildContext& ctxt, 
        ...
        ComPtr<IDxcResult>& outResult)
    {
        ...
        {
            BuildError(ctxt) << "Failed to compile shader:" << std::endl << errors->GetStringPointer() << std::endl;
            return false;
        }
        ...
        Vector<std::string>& outFiles)
    {
        ...
        std::filesystem::path sourceShaderPath = std::filesystem::absolute(MakeRelativeTo(ctxt.file, sourceFile));
        ...
        {
            BuildError(ctxt) << "Failed to load shader source file: '" << sourceShaderPath << "'" << std::endl;
            return false;
        }
        ...
        for (const EntryPoint& entryPoint : entryPoints)
        {
            ...
            if (!CompileShader(ctxt, compiler.Get(), includeHandler.Get(), sourceShaderPath.wstring(), sourceBuffer, wArgs, entryPoint, api, result.result))
            {
                return false;
            }
        ...
        ParseIncludeDependencies(ctxt, compiler.Get(), includeHandler.Get(), sourceShaderPath.wstring(), sourceBuffer, wArgs, outFiles);
        WriteShaderPDBs(ctxt, results, entryPoints, api, outFiles);
    ...
    int BuildShader(const DataBuildContext& ctxt, toml::parse_result& root, Vector<std::string>& outFiles)
    {
        if (!root["shader"])
        {
            BuildError(ctxt) << "No [shader] table found" << std::endl;
            return 1;
        }

        auto shader = root["shader"];
        if (!ValidateTable(ctxt, *shader.node(), SHADER_TABLE_SCHEMA))
        {
            return 1;
        }
        ...
        if (defines)
        {
            ...
            if (!ValidateArray(ctxt, "defines", defineArr, toml::node_type::string))
            {
                return 1;
            }
        ...
        if (includeDirs)
        {
            ...
            if (!ValidateArray(ctxt, "include_dirs", includeArr, toml::node_type::string))
            {
                return 1;
            }
        ...
        if (!vertexRasterProgram &&
            ...
            !rayTracingLib)
        {
            BuildError(ctxt) << "No shader program or library specified." << std::endl;
            return 1;
        }
        
        outFiles.push_back(MakeRelativeTo(ctxt.file, shader["source"].value_or(""sv)).string());

        Vector<EntryPoint> entryPoints;
        if (vertexRasterProgram)
        {
            if (!ValidateTable(ctxt, *vertexRasterProgram.node(), VERTEX_RASTER_PROGRAM_TABLE_SCHEMA))
            {
                return 1;
            }
        ...
        {
            if (!ValidateTable(ctxt, *meshRasterProgram.node(), MESH_RASTER_PROGRAM_TABLE_SCHEMA))
            {
                return 1;
            }
        ...
        {
            if (!ValidateTable(ctxt, *computeProgram.node(), COMPUTE_PROGRAM_TABLE_SCHEMA))
            {
                return 1;
            }
        ...
        {
            if (!ValidateTable(ctxt, *rayTracingLib.node(), RAY_TRACING_LIBRARY_TABLE_SCHEMA))
            {
                return 1;
            }
        ...
        for (int i = 0; i < CountOf<TargetAPI>(); ++i)
        {
            ...
            if (!CompileShadersForTargetAPI(ctxt, api, shader["source"].value_or(L""), entryPoints, wDefines, wIncludeDirs, results, outFiles))
            {
                return 1;
            }

            if (!WriteShaderBytecodeToHeader(ctxt, results, entryPoints, api, outFiles))
            {
                return 1;
            }
        }
